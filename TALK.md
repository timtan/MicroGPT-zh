...
- **切、拼的動作上一張（圖 4b-0）看過了**，這張只回答「為什麼」：
  一次 softmax 只能分出**一組**比例＝一個焦點，一個人只能盯一個地方。
...
- 所以切 head 就是**把最後那三個加號拿掉**。參數一個沒多、乘法一次沒多，
  ...
  差別 100% 來自 **softmax 套在加總之前還是之後**——非線性擺的位置不同，結果就完全不同。
  （這是預 4「非線性是唯一能製造分情況的地方」的第二次現身。）
...
- **它是 wte 的鏡像。** `wte` 是 id → 向量，`lm_head` 是向量 → 27 個分數，
  ...
  順帶一個細節（**回收預 3a 埋的那句**）：內積大小＝方向像不像 × 這一列本身數字有多大。
  所以常見字母只要把自己那一列整個放大，不管 x 指哪邊都先拿一份基礎分——
  這就是模型的**先天票數**。
  （別直接講「bias」——這份程式根本沒有 bias 參數，這個詞全場也沒定義過。
  真要提就補一句：其他框架會另外開一個 bias 參數做這件事，這裡它藏在 lm_head 的列裡。）
...
```
  ①　前向：用一個只有兩步的迷你算式
     ...
     [w = 2] × [x = 3]  ──▶  [b = 6]        ← 中間值
     [b = 6] ＋ [c = 1]  ──▶  [L = 7]        ← 就當它是 loss

     算的同時，兩步各留下一張便條（_local_grads）：
       乘法 → (3, 2)    ＝對方的數值：w 動 1 格，b 動 3 格
...
⋯
172      loss.backward()                                 # 圖 9.5 那張手算表，跑在 4,192 個參數上
⋯
...
| 24 | 圖 8 | 唯一的隨機性 |
| 25 | 圖 9 | 自迴歸，收攏全局 |
| 26 | 圖 9.5 | 便條怎麼往回傳：手算一次（預 5／圖 0／預 4 埋的帳，在這裡還） |
| 27 | 圖 10 | 這一切是怎麼學來的 |
