...
> **這頁是全場唯一一筆真正還過的大帳。**
> 預 5、圖 0、預 4 各埋過一次「圖 9.5 收」——如果不收，聽眾會帶著
...
- **回收預 4**：relu 的便條是 0 或 1——沒開口的偵測器，這一輪完全不會被調整。
...
| 24 | 圖 8 | 此生成流程唯一的隨機選擇 |
| 25 | 圖 9 | 自迴歸，收攏全局 |
| 26 | 圖 9.5 | 從生成回到學習：手算一次便條回傳（預 5／圖 0／預 4 埋的帳，在這裡還） |
| 27 | 圖 10 | 這一切是怎麼學來的 |
...
**圖 9.5 不可砍**：預 5、圖 0、預 4 都跟聽眾借過「等圖 9.5」，
